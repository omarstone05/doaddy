...
class EnhancedDataUploadController extends Controller
{
     ...
     * 
     * Handles both:
     * - Incoming invoices (bills from vendors) → Creates MoneyMovement expense
     * - Outgoing invoices (to customers) → Could create Invoice record (future enhancement)
     * 
     * Currently treats all invoices as expenses (bills) which is the most common case
     * when importing documents. Outgoing invoices are typically created manually.
     */
    protected function importInvoice($user, $organizationId, array $data): array
    {
        // Check if this looks like an outgoing invoice (has customer info, items, etc.)
        $hasCustomerInfo = !empty($data['customer_name']) || !empty($data['customer_id']) || !empty($data['customer']);
        $hasItems = !empty($data['items']) && is_array($data['items']) && count($data['items']) > 0;
        $isOutgoingInvoice = $hasCustomerInfo && $hasItems && empty($data['vendor']) && empty($data['merchant']);
        
        if ($isOutgoingInvoice) {
            // TODO: Future enhancement - create Invoice record for outgoing invoices
            // For now, still treat as expense to ensure data is captured
            \Log::info('Outgoing invoice detected but creating as expense', ['data' => $data]);
        }
        
        // Treat as expense (incoming bill from vendor)
