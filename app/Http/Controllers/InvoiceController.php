...
class InvoiceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Filter by customer_id if provided (for API requests)
        ...
        // If this is an API/JSON request (e.g., from axios), return JSON
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Ensure invoice_number is set (boot method should handle this, but double-check)
            if (empty($invoice->invoice_number)) {
                $invoice->invoice_number = Invoice::generateInvoiceNumber($organizationId);
                $invoice->save();
            }
            ...
            // Award XP for creating invoice (after successful commit)
            if (config('features.gamification', true)) {
                try {
                    ...
                    $gamificationService->awardXP(
                        ...
                        'invoice_issued',
                        $organizationId,
                        ['invoice_id' => $invoice->id, 'amount' => $totalAmount]
                    );
                } catch (\Exception $e) {
