...
class SaleController extends Controller
{
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Award XP for recording sale (after successful commit)
            if (config('features.gamification', true)) {
                try {
                    $gamificationService = app(\App\Services\Addy\GamificationService::class);
                    
                    // Determine XP reason based on sale characteristics
                    $xpReason = 'sale_recorded';
                    ...
                    $gamificationService->awardXP(
                        ...
                        $xpReason,
                        $organizationId,
                        ['sale_id' => $sale->id, 'amount' => $totalAmount]
                    );
                    
                    // Check for repeat customer bonus
                    if ($validated['customer_id']) {
                        $previousSalesCount = Sale::where('organization_id', $organizationId)
                    ...
                    \Log::warning('Failed to award XP for sale', ['error' => $e->getMessage()]);
                }
            }
