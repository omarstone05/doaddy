...
class AddyCommandParser
{
    ...
    {
        $keywords = ['focus', 'priority', 'priorities', 'should i', 'what should', 'today', 'do next', 'work on', 'task', 'agenda'];
        return $this->containsAny($message, $keywords);
    }
    ...
    {
        $keywords = [
            'insight', 'insights', 'issue', 'issues', 'alert', 'alerts', 'recommendation',
            'worry', 'worried', 'concern', 'concerns', 'problem', 'problems',
            'risk', 'risks', 'attention', 'urgent', 'bad', 'wrong', 'warning',
            'opportunity', 'opportunities', 'notice', 'status'
        ];
        return $this->containsAny($message, $keywords);
    }
...
protected function extractInvoiceParameters(string $message): array
    {
        ...
        // Improved pattern to handle "for [customer] for [product]" - stop at second "for"
        // Pattern: "for tobo industries for website design" -> customer: "tobo industries", product: "website design"
        if (preg_match('/(?:for|to)\s+([a-z\s]+?)(?:\s+for\s+([a-z\s]+?)|(?:\s+(?:from|on|dated|invoice|amount|\$)|$|,|\.))/i', $message, $matches)) {
            ...
            if (isset($matches[2]) && !empty(trim($matches[2]))) {
                $params['product_name'] = trim($matches[2]);
                $params['description'] = trim($matches[2]);
            }
        } else {
            // Fallback: simpler pattern if no second "for"
            if (preg_match('/(?:for|to)\s+([a-z\s]+?)(?:\s+(?:from|on|dated|invoice)|$|,|\.)/i', $message, $matches)) {
                $params['customer_name'] = trim($matches[1]);
            }
        }
        
        // Extract product/service name if mentioned separately
        // Pattern: "for [product]" or "product: [name]" or "service: [name]"
        if (!isset($params['product_name'])) {
            if (preg_match('/(?:product|service|item|description):\s*([a-z\s]+?)(?:\s|$|,|\.)/i', $message, $matches)) {
                $params['product_name'] = trim($matches[1]);
                $params['description'] = trim($matches[1]);
            } elseif (preg_match('/\b(for|of)\s+([a-z\s]{3,}?)(?:\s+(?:amount|\$|cost|price)|$|,|\.)/i', $message, $matches)) {
                // Try to catch product after "for" or "of" if customer already extracted
                $potentialProduct = trim($matches[2]);
                if (!empty($potentialProduct) && !isset($params['customer_name'])) {
                    $params['product_name'] = $potentialProduct;
                    $params['description'] = $potentialProduct;
                }
            }
        }
        
        // Extract amount - handle formats like "10 000", "10,000", "10000", "$10000", "ZMW 10000"
        // Pattern: number with optional spaces, commas, or decimal, currency symbols
        if (preg_match('/(?:amount|total|cost|price|value)[:\s]*\$?\s*(\d{1,3}(?:\s+\d{3})*(?:\.\d{2})?|\d{1,3}(?:,\d{3})*(?:\.\d{2})?|\d+(?:\.\d{2})?)/i', $message, $matches)) {
            $amount = str_replace([' ', ','], '', $matches[1]);
            $params['total_amount'] = (float) $amount;
        } elseif (preg_match('/\$?\s*(\d{1,3}(?:\s+\d{3})*(?:\.\d{2})?|\d{1,3}(?:,\d{3})*(?:\.\d{2})?|\d+(?:\.\d{2})?)/', $message, $matches)) {
