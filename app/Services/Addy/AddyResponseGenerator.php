...
class AddyResponseGenerator
{
    ...
    protected function parseBankStatementTextManually(string $text): array
    {
        try {
            ...
            $systemMessage = "You are a specialized financial data extraction AI. 
            Your task is to extract bank transactions from raw text pasted from a bank statement PDF or website.
            ...
            Output MUST be a valid JSON object with this structure:
            {
                ...
                \"currency\": \"extracted currency code (e.g. ZMW, USD) or ZMW\",
                \"transactions\": [
                    {
            ...
            RULES:
            1. 'type' is 'credit' for money coming in, 'debit' for money going out.
            2. 'flow_type' is 'income' for credits, 'expense' for debits.
            3. Ignore headers, footers, and page numbers.
            4. If the text contains no transactions, return empty transactions array.
            5. Extract as many details as possible.";
            
            $response = $ai->chat([
                ['role' => 'system', 'content' => $systemMessage],
                ['role' => 'user', 'content' => "Extract transactions from this text:\n\n" . substr($text, 0, 10000)] // Limit text length
            ], 2000);
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                \Log::error('Failed to parse AI bank statement response', ['error' => json_last_error_msg(), 'content' => $content]);
                return [];
            }
            ...
            \Log::error('AI Bank Statement Parsing failed', ['error' => $e->getMessage()]);
            return [];
        }
    }
    ...
    protected function enrichActionFromHistory(array $intent, array $chatHistory, string $currentMessage, array $extractedData = []): array
    {
        ...
        if ($intent['action_type'] === 'create_invoice') {
            ...
            // Extract customer name from message if not set - improved pattern to handle "for [customer] for [product]"
            if (!isset($intent['parameters']['customer_id']) && !isset($intent['parameters']['customer_name'])) {
                if (preg_match('/(?:for|to)\s+([a-z\s]+?)(?:\s+for\s+([a-z\s]+?)|(?:\s+(?:from|on|dated|invoice|amount|\$)|$|,|\.))/i', $fullContext, $matches)) {
                    $intent['parameters']['customer_name'] = trim($matches[1]);
                    // If there's a second "for", that's the product/service
                    if (isset($matches[2]) && !empty(trim($matches[2]))) {
                        $intent['parameters']['product_name'] = trim($matches[2]);
                        $intent['parameters']['description'] = trim($matches[2]);
                    }
                } elseif (preg_match('/(?:for|to)\s+([a-z\s]+?)(?:\s+(?:from|on|dated|invoice)|$|,|\.)/i', $fullContext, $matches)) {
                    $intent['parameters']['customer_name'] = trim($matches[1]);
                }
            }
            ...
            // Extract customer name from history if not already set - improved pattern
            if (!isset($intent['parameters']['customer_id']) && !isset($intent['parameters']['customer_name'])) {
                if (preg_match('/(?:for|to)\s+([a-z\s]+?)(?:\s+for\s+([a-z\s]+?)|(?:\s|$|,|\.|for|invoice|amount|\$))/i', $fullContext, $matches)) {
                    $intent['parameters']['customer_name'] = trim($matches[1]);
                    // If there's a second "for", that's the product/service
                    if (isset($matches[2]) && !empty(trim($matches[2]))) {
                        $intent['parameters']['product_name'] = trim($matches[2]);
                ...
                } elseif (preg_match('/(?:for|to)\s+([a-z\s]+?)(?:\s|$|,|\.|for|invoice)/i', $fullContext, $matches)) {
                    $intent['parameters']['customer_name'] = trim($matches[1]);
                }
            }
    ...
    protected function handleActionRequest(array $intent, string $userMessage): array
    {
        ...
        try {
            ...
            // For reports, don't show the preview description and items in the chat
            // The actual report will be shown after execution
            $isReport = stripos($preview['title'] ?? '', 'report') !== false || 
                       stripos($intent['action_type'] ?? '', 'report') !== false;

            if ($isReport) {
                // For reports, just show the title and a simple message
                $response = "**{$preview['title']}**\n\n";
                $response .= "I'll generate this report for you. Click 'Confirm & Execute' to see the results.\n";
            } else {
                // For other actions, show full preview
                $response = "**{$preview['title']}**\n\n";
    ...
    protected function getFallbackResponse(array $intent, ?array $dataContext): string
    {
        ...
        switch ($intentType) {
            ...
            case 'query_insights':
                if ($dataContext && isset($dataContext['insights'])) {
                    $insights = $dataContext['insights'];
                    if (empty($insights)) {
                        return "✨ All clear! No urgent insights right now. Your business is running smoothly.";
                    }
                    
                    $response = "**Active Insights:**\n\n";
                    ...
                    if (count($insights) > 3) {
                        $more = count($insights) - 3;
                        $response .= "_+ {$more} more insight(s)_\n";
                    }
                    return $response;
                }
                return "I can help you uncover insights about your business. Check the Insights panel for details.";
            case 'query_focus':
                if ($dataContext && isset($dataContext['focus_area'])) {
                    $response = "**Your Focus Today:**\n\n";
                    $response .= "**Area:** {$dataContext['focus_area']}\n";
                    if (!empty($dataContext['context'])) {
                        $response .= "**Context:** {$dataContext['context']}\n";
                    }
                    return $response;
                }
                return "I can help you prioritize your day. Ask me what to focus on!";
            case 'greeting':
                return "Hello! I'm Addy, your AI business assistant. How can I help you today?";
            default:
                // Try to be smarter about the fallback even if intent was general
                ...
                // If we have a cultural engine, use it to give a more personality-driven "I don't know"
                try {
                    $culturalEngine = new \App\Services\Addy\AddyCulturalEngine($this->organization, $this->user);
                    $tone = $culturalEngine->getTone();
                    
                    if ($tone === 'sassy') {
                        return "Look, I'm having a bit of a moment connecting to my brain. Check your internet or API keys, darling. In the meantime, I can still show you your data if you ask nicely.";
                    } elseif ($tone === 'casual') {
                        return "My brain is a bit foggy right now (API connection issue). But I can still pull up your data! Try asking about 'cash', 'sales', or 'expenses'.";
                    }
                } catch (\Exception $e) {
                    // Ignore
                }
                
                return "I'm having trouble connecting to my AI brain right now, so I can't have a full conversation. \n\nHowever, I can still fetch your data! Try asking:\n- \"Show me cash position\"\n- \"List my expenses\"\n- \"What are my sales?\"\n- \"Show me insights\"";
        }
    }
    ...
    protected function buildConversationalSystemMessage($state, $thought, string $tone, ?array $dataContext, AddyCulturalEngine $culturalEngine, ?array $historicalContext = null): string
    {
        ...
        $message .= "TONE AND STYLE INSTRUCTIONS:\n";
        $message .= $culturalEngine->getToneInstructions();
        $message .= "\n\n";
