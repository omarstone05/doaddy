...
class RecordInvoicePaymentAction extends BaseAction
{
    ...
    public function execute(): array
    {
        ...
        // The PaymentAllocation observer will handle updating the invoice paid_amount
        // We just need to refresh the invoice to get the latest values
        $invoice->refresh();

        // Check if fully paid
        if ($invoice->balance <= 0) {
            $invoice->update(['paid_at' => now(), 'status' => 'paid']);
        } else {
