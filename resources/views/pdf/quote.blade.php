...
<body>
    ...
    <div class="summary-section">
        <div class="summary-card">
            ...
            <div class="summary-value">{{ $quote->customer->name }}</div>
            @if($quote->customer->email)
                <div class="summary-value-secondary">{{ $quote->customer->email }}</div>
            @endif
            @if($quote->customer->phone)
                <div class="summary-value-secondary">{{ $quote->customer->phone }}</div>
            @endif
        </div>
        <div class="summary-card">
            <div class="summary-label">Quote Date</div>
            <div class="summary-value">{{ \Carbon\Carbon::parse($quote->quote_date)->format('F d, Y') }}</div>
            @if($quote->expiry_date)
                <div class="summary-label" style="margin-top: 12px;">Expiry Date</div>
                <div class="summary-value">{{ \Carbon\Carbon::parse($quote->expiry_date)->format('F d, Y') }}</div>
            @endif
        </div>
        <div class="summary-card" style="text-align: right;">
            <div class="summary-label">Total Amount</div>
            <div class="summary-value-large">{{ number_format($quote->total_amount, 2) }} {{ $organization->currency ?? 'ZMW' }}</div>
        </div>
    </div>
    ...
    <div class="totals-section">
        <div class="totals-row">
            ...
            <span class="totals-row-value">{{ number_format($quote->subtotal, 2) }} {{ $organization->currency ?? 'ZMW' }}</span>
        </div>
        @if($quote->tax_amount > 0)
            <div class="totals-row">
                <span class="totals-row-label">Tax:</span>
                <span class="totals-row-value">{{ number_format($quote->tax_amount, 2) }} {{ $organization->currency ?? 'ZMW' }}</span>
            </div>
        @endif
        @if($quote->discount_amount > 0)
            <div class="totals-row">
                <span class="totals-row-label">Discount:</span>
                <span class="totals-row-value" style="color: #dc2626;">-{{ number_format($quote->discount_amount, 2) }} {{ $organization->currency ?? 'ZMW' }}</span>
            </div>
        @endif
