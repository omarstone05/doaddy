...
export default function BudgetsEdit({ budget }) {
    ...
    useEffect(() => {
        if (data.start_date && data.period && !data.end_date) {
            setData('end_date', calculateEndDate(data.start_date, data.period));
        }
    }, [data.start_date, data.period]);

    return (
        <SectionLayout sectionName="Budgets">
            <Head title={`Edit Budget - ${budget.name}`} />
            <div className="max-w-2xl mx-auto">
                <div className="mb-6">
                    ...
                    <p className="text-gray-500 mt-1">{budget.name}</p>
                </div>
