...
import { ArrowLeft, Save, Package, Droplets, ToggleLeft, ToggleRight } from 'lucide-react';

export default function MaterialsEdit({ material, inkConfigurations, materialTypes }) {
    const { data, setData, put, processing, errors } = useForm({
        name: material.name || '',
        material_type: material.material_type || 'vinyl',
        roll_width: material.roll_width || '',
        roll_length: material.roll_length || '',
        material_cost: material.material_cost || '',
        off_cut_cost: material.off_cut_cost || '7.00',
        is_active: material.is_active ?? true,
        notes: material.notes || '',
        ink_configuration_ids: material.ink_configurations?.map(ic => ic.id) || [],
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(`/print-shop/materials/${material.id}`);
    };
    ...
    return (
        <PrintShopLayout>
            ...
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h2 className="text-2xl font-black text-gray-900 tracking-tight">Edit Material</h2>
                    <p className="text-gray-500 mt-1">{material.name}</p>
                </div>
            </div>
