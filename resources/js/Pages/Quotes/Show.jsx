...
export default function QuotesShow({ quote }) {
    ...
    // Normalize quotation data to match quote structure
    const normalizedQuote = {
        ...quote,
        quote_number: quote.quotation_number || quote.quote_number || '',
        quote_date: quote.issue_date || quote.quote_date || new Date().toISOString(),
        expiry_date: quote.valid_until || quote.expiry_date,
        total_amount: quote.total || quote.total_amount || 0,
        notes: quote.internal_notes || quote.notes,
        terms: quote.terms_and_conditions || quote.terms,
        invoice_id: quote.converted_to_invoice_id || quote.invoice_id,
    ...
    const handleConvert = () => {
        if (confirm('Convert this quotation to an invoice?')) {
            router.post(`/quotations/${quote.id}/convert`);
        }
    };
        ...
        <AuthenticatedLayout>
            ...
            <div className="max-w-5xl mx-auto">
                        ...
                        <div className="flex items-start justify-between mb-6">
                            ...
                            <div className="flex flex-col gap-2 items-end">
                                ...
                                <div className="flex gap-2">
                                    <a
                                        href={`/quotations/${quote.id}/download-pdf`}
                                        target="_blank"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-gray-200">
                            <div>
                                <h3 className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">
                                    {normalizedQuote.customer ? 'Customer' : 'Prospect'}
                                </h3>
                                <p className="text-sm font-medium text-gray-900">
                                    {normalizedQuote.customer?.name || normalizedQuote.prospect?.company_name || normalizedQuote.prospect?.name || '-'}
                                </p>
                                {(normalizedQuote.customer?.email || normalizedQuote.prospect?.email) && (
                                    <p className="text-sm text-gray-600 mt-1">{normalizedQuote.customer?.email || normalizedQuote.prospect?.email}</p>
                                )}
                                {(normalizedQuote.customer?.phone || normalizedQuote.prospect?.phone) && (
                                    <p className="text-sm text-gray-600">{normalizedQuote.customer?.phone || normalizedQuote.prospect?.phone}</p>
                                )}
                            </div>
                            <div>
                                <h3 className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Quote Date</h3>
                                <p className="text-sm font-medium text-gray-900">{new Date(normalizedQuote.quote_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                                {normalizedQuote.expiry_date && (
                                    <>
                                        <h3 className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2 mt-3">Expiry Date</h3>
                                        <p className="text-sm font-medium text-gray-900">{new Date(normalizedQuote.expiry_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                                    </>
                                )}
                            </div>
                            <div>
                                <h3 className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Total Amount</h3>
                                <p className="text-2xl font-bold text-gray-900">{formatCurrency(normalizedQuote.total_amount)}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="mb-6">
                        <table className="w-full">
                            ...
                            <tbody className="divide-y divide-gray-200">
                                {normalizedQuote.items?.map((item, index) => (
                                    <tr key={index}>
                                        ...
                                        <td className="px-4 py-3 text-right font-medium text-gray-900">{formatCurrency(item.total || (item.quantity * item.unit_price))}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                        ...
                        <div className="flex justify-end">
                            <div className="w-full max-w-xs space-y-2">
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="text-gray-900 font-medium">{formatCurrency(normalizedQuote.subtotal || 0)}</span>
                                </div>
                                {(normalizedQuote.tax_amount || 0) > 0 && (
                                    <div className="flex justify-between text-sm">
                                        <span className="text-gray-600">Tax:</span>
                                        <span className="text-gray-900 font-medium">{formatCurrency(normalizedQuote.tax_amount)}</span>
                                    </div>
                                )}
                                {(normalizedQuote.discount_amount || 0) > 0 && (
                                    <div className="flex justify-between text-sm">
                                        <span className="text-gray-600">Discount:</span>
                                        <span className="text-red-600 font-medium">-{formatCurrency(normalizedQuote.discount_amount)}</span>
                                    </div>
                                )}
                                <div className="flex justify-between text-lg font-bold pt-2 border-t-2 border-gray-300">
                                    <span>Total:</span>
                                    <span>{formatCurrency(normalizedQuote.total_amount)}</span>
                                </div>
                            </div>
                        </div>
                    </div>

                    {normalizedQuote.notes && (
                        <div className="mt-6 pt-4 border-t border-gray-200">
                            <h3 className="text-sm font-medium text-gray-500 mb-2">Notes</h3>
                            <p className="text-gray-900">{normalizedQuote.notes}</p>
                        </div>
                    )}

                    {normalizedQuote.terms && (
                        <div className="mt-6 pt-4 border-t border-gray-200">
                            <h3 className="text-sm font-medium text-gray-500 mb-2">Terms & Conditions</h3>
                            <p className="text-gray-900">{normalizedQuote.terms}</p>
                        </div>
                    )}
                </div>
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-6 mb-4">
                    <FileUpload
                        attachableType="App\Models\Quotation"
