...
export default function QuotesEdit({ quote, customers: initialCustomers = [], products: initialProducts = [] }) {
    ...
    const [items, setItems] = useState(
        quote?.items?.map(item => ({
            ...
            goods_service_id: item.goods_service_id || item.product_id || '',
        })) || [{ name: '', description: '', quantity: 1, unit_price: 0, goods_service_id: '' }]
    );

    const { data, setData, put, processing, errors } = useForm({
        customer_id: quote?.customer_id || '',
        quote_date: formatDateForInput(quote?.issue_date || quote?.quote_date),
        expiry_date: formatDateForInput(quote?.valid_until || quote?.expiry_date),
        items: [],
    ...
    const submit = (e) => {
        ...
        router.put(`/quotations/${quote.id}`, data, {
            preserveScroll: true,
            onError: (errors) => {
                console.error('Quote update errors:', errors);
            },
        });
    };

    const canEdit = !quote?.converted_to_invoice_id && !quote?.invoice_id; // Can't edit if converted to invoice
        ...
        <AuthenticatedLayout>
            ...
            <div className="max-w-4xl mx-auto ">
                <div className="mb-6">
                    ...
                    <p className="text-gray-500 mt-1">Quote #{quote?.quotation_number || quote?.quote_number}</p>
                </div>

                {!canEdit && (
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                        <p className="text-yellow-800">
                            This quotation cannot be edited because it has been converted to an invoice.
                        </p>
                    </div>
                )}
