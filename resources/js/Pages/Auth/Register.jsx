...
export default function Register() {
    ...
    // WhatsApp registration form data
    ...
    const sendWhatsAppCode = async (e) => {
        ...
        try {
            const response = await axios.post('/register/whatsapp/send-code', {
            ...
            if (response.data.success) {
                ...
                if (response.data.test_mode && response.data.code) {
    ...
    const verifyWhatsAppCode = async (e) => {
        ...
        try {
            const response = await axios.post('/register/whatsapp/verify', {
                phone_number: whatsappPhone,
                code: whatsappCode,
            }, {
                headers: {
    ...
    // Submit WhatsApp registration
    const submitWhatsAppRegistration = (e) => {
        e.preventDefault();
        router.post('/register/whatsapp', whatsappFormData);
    };
            ...
            <div className="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
                ...
                <div className="relative z-10 max-w-md w-full">
                    <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/50 p-8 space-y-8">
                        ...
                        {/* WhatsApp Registration Tab */}
                        {activeTab === 'whatsapp' && (
                            <>
                            {!codeSent ? (
                                <form className="space-y-6" onSubmit={sendWhatsAppCode}>
                                    <div>
                                        <label htmlFor="whatsapp-phone-register" className="block text-sm font-medium text-gray-700 mb-1">
                                            Phone Number
                                        </label>
                                        <input
                                            id="whatsapp-phone-register"
                                            type="tel"
                                            value={whatsappPhone}
                                ...
                                <form className="space-y-6" onSubmit={verifyWhatsAppCode}>
                                    ...
                                    {testModeCode && (
                                        <div className="bg-teal-50 border-2 border-teal-300 text-teal-800 px-4 py-3 rounded-lg">
                                            <p className="font-semibold mb-1">Test Mode - Your Code:</p>
                                            <p className="text-2xl font-mono font-bold text-center">{testModeCode}</p>
                                        </div>
                                    )}

                                    <div>
                                        <label htmlFor="whatsapp-code-register" className="block text-sm font-medium text-gray-700 mb-1">
                                            Enter Verification Code
                                        </label>
                                        <input
                                            id="whatsapp-code-register"
                                            type="text"
                                            value={whatsappCode}
                                ...
                                <form className="space-y-6" onSubmit={submitWhatsAppRegistration}>
                                    {whatsappMessage && (
                                        <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-lg text-sm">
                                            {whatsappMessage}
                                        </div>
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            <label htmlFor="whatsapp-org-name" className="block text-sm font-medium text-gray-700 mb-1">
                                                Organization Name
                                            </label>
                                            <input
                                                id="whatsapp-org-name"
                                                type="text"
                                                value={whatsappFormData.organization_name}
                                                onChange={(e) => setWhatsappFormData(prev => ({ ...prev, organization_name: e.target.value }))}
                                                className="w-full px-4 py-3 bg-white/80 backdrop-blur-sm border border-teal-200/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-400/50 focus:border-teal-300 text-gray-900 placeholder:text-gray-400"
                                                placeholder="Your business name"
                                                required
                                            />
                                        ...
                                        <div>
                                            <label htmlFor="whatsapp-password" className="block text-sm font-medium text-gray-700 mb-1">
                                                Password
                                            </label>
                                            <input
                                                ...
                                                onChange={(e) => setWhatsappFormData(prev => ({ ...prev, password: e.target.value }))}
                                                className="w-full px-4 py-3 bg-white/80 backdrop-blur-sm border border-teal-200/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-400/50 focus:border-teal-300 text-gray-900 placeholder:text-gray-400"
                                                placeholder="••••••••"
                                                required
                                            />
                                        ...
                                        <div>
                                            <label htmlFor="whatsapp-password-confirmation" className="block text-sm font-medium text-gray-700 mb-1">
                                                Confirm Password
                                            </label>
                                            <input
                                                id="whatsapp-password-confirmation"
                                                type="password"
                                                value={whatsappFormData.password_confirmation}
                                                onChange={(e) => setWhatsappFormData(prev => ({ ...prev, password_confirmation: e.target.value }))}
                                                className="w-full px-4 py-3 bg-white/80 backdrop-blur-sm border border-teal-200/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-400/50 focus:border-teal-300 text-gray-900 placeholder:text-gray-400"
                                                placeholder="••••••••"
                                                required
                                            />
