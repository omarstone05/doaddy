...
export default function ActionConfirmation({ action, onConfirm, onCancel, messageId, onUpdateMessage }) {
    ...
    if (displayResult || isExecuted) {
        ...
        // Show actual report if available
        const report = displayResult?.result?.report;
            ...
            <div className={`p-4 rounded-lg mt-3 ${displayResult?.success ? 'bg-green-50/80 backdrop-blur-sm border border-green-300/50' : 'bg-red-50/80 backdrop-blur-sm border border-red-300/50'}`}>
                ...
                {displayResult?.success && report && (
                    <div className="mt-4 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
                        ...
                        {report.data?.cash_flow && (
                            <div className="mb-4">
                                ...
                                <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                                    {report.data.cash_flow.income_total !== undefined && (
                                        <div className="flex justify-between mb-2">
                                            <span className="text-sm text-gray-600">Total Income:</span>
                                            <span className="text-sm font-semibold text-green-600">
                                                {typeof report.data.cash_flow.income_total === 'number' 
                                                    ? new Intl.NumberFormat('en-ZM', {
                                                        style: 'currency',
                                                        currency: 'ZMW',
                                                        minimumFractionDigits: 2
                                                    }).format(report.data.cash_flow.income_total)
                                                    : report.data.cash_flow.income_total}
                                            </span>
                                        </div>
                                    )}
                                    {report.data.cash_flow.expense_total !== undefined && (
                                        <div className="flex justify-between mb-2">
                                            <span className="text-sm text-gray-600">Total Expenses:</span>
                                            <span className="text-sm font-semibold text-red-600">
                                                {typeof report.data.cash_flow.expense_total === 'number'
                                                    ? new Intl.NumberFormat('en-ZM', {
                                                        style: 'currency',
                                                        currency: 'ZMW',
                                                        minimumFractionDigits: 2
                                                    }).format(report.data.cash_flow.expense_total)
                                                    : report.data.cash_flow.expense_total}
                                            </span>
                                        </div>
                                    )}
                                    {report.data.cash_flow.net_cash_flow !== undefined && (
                                        <div className="flex justify-between pt-2 border-t border-gray-300">
                                            <span className="text-sm font-semibold text-gray-700">Net Cash Flow:</span>
                                            <span className={`text-sm font-bold ${
                                                (typeof report.data.cash_flow.net_cash_flow === 'number' && report.data.cash_flow.net_cash_flow >= 0) 
                                                    ? 'text-green-600' 
                                                    : 'text-red-600'
                                            }`}>
                                                {typeof report.data.cash_flow.net_cash_flow === 'number'
                                                    ? new Intl.NumberFormat('en-ZM', {
                                                        style: 'currency',
                                                        currency: 'ZMW',
                                                        minimumFractionDigits: 2
                                                    }).format(report.data.cash_flow.net_cash_flow)
                                                    : report.data.cash_flow.net_cash_flow}
                                            </span>
                                        </div>
                                    )}
                                </div>
                        ...
                        {/* Expense Breakdown */}
                        {report.data?.expenses?.top_categories && report.data.expenses.top_categories.length > 0 && (
                            <div className="mb-4">
                                <h4 className="text-sm font-semibold text-gray-700 mb-2">Top Expense Categories</h4>
                                <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                                    <div className="space-y-2">
                                        {report.data.expenses.top_categories.map((category, index) => (
    ...
    return (
        <div className="bg-teal-50 border border-teal-200 rounded-lg p-4 mt-3">
            <div className="mb-3">
                ...
                {/* Only show description if it's not a report (to avoid showing the "Generate...for..." text) */}
                {preview.title && !preview.title.toLowerCase().includes('report') && (
            ...
            {/* Preview Items - Only show for non-report actions */}
            {preview.items && preview.items.length > 0 && !preview.title?.toLowerCase().includes('report') && (
