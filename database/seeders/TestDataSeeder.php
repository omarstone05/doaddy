...
class TestDataSeeder extends Seeder
{
    ...
    private function createSales($organization, $customers, $products, $accounts, $teamMember): array
    {
        ...
        for ($i = 0; $i < $numSales; $i++) {
            ...
            // Generate sale number manually to ensure it's set
            $date = $saleDate->format('Ymd');
            $prefix = "SALE-{$date}-";
            $lastSale = Sale::where('organization_id', $organization->id)
            ...
            $saleNumber = $prefix . str_pad($newNumber, 4, '0', STR_PAD_LEFT);
    ...
    private function createQuotes($organization, $customers, $products): array
    {
        ...
        for ($i = 0; $i < $numQuotes; $i++) {
            ...
            $date = $quoteDate->format('Ymd');
            $prefix = "QUOTE-{$date}-";
            $lastQuote = Quote::where('organization_id', $organization->id)
